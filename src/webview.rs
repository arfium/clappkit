...
//!     is a separate invocation and keeps working regardless: `<cli> --help` and the
//!     app's read-only verbs need no webview at all.
//!
...
//!
//! [`crate::role::main_dispatch`] calls this before the GUI role starts, so a clapp gets
//! it without wiring anything. It was written three times in three apps before it moved
//! here, which is the usual sign that it was platform plumbing all along.
...
fn vendored() -> Option<std::path::PathBuf> {
    let dir = crate::paths::install_root().join("vendor").join("webview2");
