...
use crate::vocab::{ConnectedAgent, SignalDecl, SignalType};
use clapp_ipc::{Error, Result};
use clapp_ipc::{connect, frame, Frame, FrameLimits, Inbox, Kind, Peer, Request, Response};
...
impl Client {
    ...
    pub async fn from_env(declared: &[SignalDecl]) -> Result<Self> {
        let identity = Identity::from_env()
            .ok_or_else(|| Error::Invalid("not launched by Clatch".into()))?;
    ...
    pub async fn connect(identity: Identity, declared: &[SignalDecl]) -> Result<Self> {
        ...
        let reply = frame::read::<_, Frame>(&mut stream, FrameLimits::control())
            .await
            .map_err(|e| Error::io(&addr, e))?
            .ok_or_else(|| Error::Invalid("host closed during register".into()))?;
    ...
    pub async fn signal_to(&self, id: &str, payload: Value, target: Vec<String>) -> Result<()> {
        let signal_type = self.declared_type(id).ok_or_else(|| {
            Error::Invalid(format!("refusing to emit undeclared signal '{id}'"))
        })?;
        self.peer
            .notify(
                method::TO_AGENT,
